...
# ----------------------------------------------------------------------
# Funciones centrales (sin cambios, ellas esperan los inputs)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# Función de Interacción con el Usuario (El Agente Inteligente)
# ----------------------------------------------------------------------
...
def get_user_input():
    ...
    # 2. Consulta de Preferencias y Restricciones (Lógica)
    preferences = {}
    
    # Preguntas para restricciones booleanas
    print("\n❓ Restricciones Dietéticas:")
    
    # Función auxiliar para manejar S/N
...
if __name__ == '__main__':
    ...
    if not results.empty:
        ...
        # 3. Ejecutar la recomendación (ML)
        best_recipe = results.iloc[0]['nombre']
        
        # 🚨 NUEVA FUNCIONALIDAD: Búsqueda en Internet
        search_online = input(f"\n💡 ¿Desea buscar las instrucciones de '{best_recipe}' en Google para ver la receta completa? (S/N): ").upper()

        if search_online == 'S':
            # Llamada a la herramienta de búsqueda con la query
            print(f"Buscando: Receta de {best_recipe}...")
            # Aquí es donde se ejecutaría la herramienta de Google Search:
            # search_tool.run(f"Receta de {best_recipe}")
            # Dado que estamos simulando la integración, mostraremos la salida.
            
            # --- SIMULACIÓN DE LA SALIDA DE BÚSQUEDA ---
            print("--- RESULTADOS DE GOOGLE SEARCH ---")
            print(f"  * [Link 1] Cómo preparar {best_recipe} paso a paso | Recetas.com")
            print(f"  * [Link 2] La mejor receta de {best_recipe} casero | CocinaFácil.net")
            print("-----------------------------------")
            
        # Pregunta de interacción para la fase de ML (el resto del código sigue igual)
